...
def preprocess_states(maze: List[List[str]]) -> tuple[frozenset[tuple[int, int]], List[str]]:
        ...
        for j in range(1, m - 1):    
            if maze[i][j] in ('S', 'B'):
                ...
                while maze[k][l-1] in ('S', 'B', 'G'):
                    l -= 1
                # Przesuwanie do dołu i w prawo (maksymalnie m razy)
                for _ in range(m):
                    if maze[k+1][l] != '#':
                        k += 1
                    if maze[k][l+1] != '#':
    ...
    path = []  # Lista do zapisywania ruchów

    # Wykonujemy 30 losowych ruchów, aktualizując stan oraz zapisując ruchy
    for _ in range(40):
        move = random.choice(list(directions.keys()))
        state = create_new_state(state, move, maze)
...
if __name__ == '__main__':
    ...
    preprocess_moves = 'U' * height + 'L' * width + 'DR' * width

    start_state, random_path = preprocess_states(maze)
    start_path = preprocess_moves + ''.join(random_path)
